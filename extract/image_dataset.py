...
class FeatureStorage:
    def __init__(self, save_dir, desc_name, split, extension, global_desc_dim, local_desc_dim, dataset_size, save_type, topk=400, debug=False):
    ...
    def save(self, feats, save_type):
        if save_type in self.storage:
            ...
            if self.debug and save_type == 'global' and self.pointer == 0:
                print(feats)   # saved passed w/o issues
...
class DataSet(torch.utils.data.Dataset):
    ...
    def __getitem__(self, indices):
        # Load the image
        im_list, scale_list = [], []
        for index in indices:   # why indices? due to sequential sampler.
            try:

...
def main():
    ...
    parser.add_argument('--imsize', type=int  # left empty with CVNet
                        # , default=770  # set to 770 for DINOv2
                        )
    ...
    # for gldv2, we need a file with image paths?
    # maybe just rewrite it in our data loading logic.
    ...
    else:  # gldv2 part, will skip gnd box
    ...
    feature_storage = FeatureStorage(save_path, args.backbone, args.split, file_name, global_dim, local_dim,
                                     len(dataset), args.desc_type, topk=topk, debug=args.debug)
